        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-6">
              ...
              <div className="bg-white/60 backdrop-blur-2xl border border-white/30 rounded-2xl shadow-sm p-6">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600 mb-4">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {new Date(currentProject.metadata.lastModified).toLocaleDateString()}
                  </span>
                </div>
                <Button onClick={() => router.push('/project')} variant="outline" className="rounded-xl w-full">
                  Manage Project
                </Button>
              </div>
            ) : (
            ...
            {(isTraining || isProcessing || currentDataset || bestModel) && (
              <div className="bg-white/60 backdrop-blur-2xl border border-white/30 rounded-2xl shadow-sm p-6 flex-1">
                ...
                <div className="space-y-3">
                  ...
                  {bestModel && (
                    <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                      ...
                      <Button size="sm" onClick={() => router.push('/train')} variant="outline" className="w-full rounded-xl">
                        View Results
                      </Button>
                    </div>
                  )}
          ...
          <div className="space-y-6 flex flex-col">
            {/* Quick Actions */}
            <div className="bg-white/60 backdrop-blur-2xl border border-white/30 rounded-2xl shadow-sm p-6 flex-1 flex flex-col">
              ...
              <div className="space-y-4 flex-1 flex flex-col justify-between">
                {quickActions.map((action) => (
                  <Link
                    key={action.title}
                    href={action.href}
                    className="group p-5 rounded-xl border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all bg-white/40 flex items-center gap-x-3"
                  >
                    <div className={`p-2 ${action.color} rounded-xl border border-gray-200`}>
          ...
          {isLoadingProjects ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {recentProjects.map((project) => (
                <div
                  key={project.id}
                  className="p-4 rounded-xl border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all bg-white/40 cursor-pointer"
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-600 mt-1 line-clamp-2">{project.description || 'No description'}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between text-xs text-gray-600 pt-3 border-t border-gray-200">
                    <span>{project.files?.length || 0} files</span>
                    <span>{new Date(project.updated_at * 1000).toLocaleDateString()}</span>
                  </div>
                </div>
              ))}
            </div>
